...
int main(int argc, char *argv[]) {
  ...
  auto laser_pub = node->create_publisher<sensor_msgs::msg::LaserScan>(
    "scan", 
    rclcpp::QoS(rclcpp::SystemDefaultsQoS()).reliable());
  ...
  while (ret && rclcpp::ok()) {
    ...
    if (laser.doProcessSimple(scan)) {
      ...
      scan_msg->header.stamp.sec = node->get_clock()->now().seconds();
      scan_msg->header.stamp.nanosec = node->get_clock()->now().nanoseconds();
