...
void* YoloObjectDetector::publishInThread() {
  ...
  /*if(boundingBox.id==0)
  {
    pub_flag = nodeHandle_.advertise<darknet_ros_msgs::BoundingBox>("/boundingBox_msg",1);
    
  	int i = 0;
    printf("probability = %f",boundingBox.probability);
  	while(i<2)
  	{
  	
  	pub_flag.publish(boundingBox);
  	usleep(100000);
  	i++;
  	}
...
if(boundingBox.id==1)
  {
      ros::Publisher pub_twist = nodeHandle_.advertise<geometry_msgs::Twist>("/smoother_cmd_vel",1000);
  ...
  if(boundingBox.id==5|boundingBox.id==4)
   {  
    ros::Publisher pub_twist = nodeHandle_.advertise<geometry_msgs::Twist>("/smoother_cmd_vel",1000);
      geometry_msgs::Twist tw; 
     tw.linear.x=0.0;
