...
void Node::Init () {
  ...
  if (publish_pointcloud_param_) {
    ...
    cloud_points_publisher_ = node_handle_.advertise<sensor_msgs::PointCloud2> (name_of_node_+"/cloud_points", 1);
  }
...
void Node::PublishCloudPoints (pcl::PointCloud<PointT>::Ptr cloud_points) {
  if (!cloud_points->empty())
  {
    ...
    pcl::toROSMsg(*cloud_points,cloudMap);
    cloudMap.header.stamp=ros::Time::now();
    cloudMap.header.frame_id  ="cloudpoint";
    cloud_points_publisher_.publish (cloudMap);
...
void Node::LoadOrbParameters (ORB_SLAM2::ORBParameters& parameters) {
  ...
  node_handle_.param(name_of_node_ + "/resolution", parameters.resolution,0.01);
  node_handle_.param(name_of_node_ + "/meank", parameters.meank, 50.0);
  node_handle_.param(name_of_node_ + "/thresh", parameters.thresh, 1.0);  
